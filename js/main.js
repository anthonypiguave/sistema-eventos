(function() {
    ...
    var pago = document.getElementById('forma');
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		if (document.getElementById('mapa')) {
            var map = L.map('mapa').setView([-2.202978, -79.894488], 17);

			L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

			L.marker([ -2.202978, -79.894488 ]).addTo(map).bindPopup('XENTURIONIT 2022').openPopup();
			// .bindTooltip('XENTURIONIT 2018, Boletos ya disponibles')
			// .openTooltip();
		}
	});
